...
class Circuit:
    ...
    def Add_R(self, splitted_line, Yn):
        node_a = int(splitted_line[1])
        node_b = int(splitted_line[2])
    ...
    def Add_GmI(self, splitted_line, Yn):
        node_a = int(splitted_line[1])
        node_b = int(splitted_line[2])
    ...
    def Add_C(self, splitted_line, Yn):
        node_a = int(splitted_line[1])
        node_b = int(splitted_line[2])
        C = float(splitted_line[3])
        
        Yn[node_a, node_a] += self.w*C*1j
        Yn[node_b, node_b] += self.w*C*1j
        Yn[node_a, node_b] += -self.w*C*1j
        Yn[node_b, node_a] += -self.w*C*1j
    ...
    def Add_L(self, splitted_line, Yn):
        node_a = int(splitted_line[1])
        node_b = int(splitted_line[2])
        L = float(splitted_line[3])
        
        Yn[node_a, node_a] += 1/(self.w*L*1j)
        Yn[node_b, node_b] += 1/(self.w*L*1j)
        Yn[node_a, node_b] += -1/(self.w*L*1j)
        Yn[node_b, node_a] += -1/(self.w*L*1j)
    ...
    def Add_K(self, splitted_line, Yn):
    ...
    def analyze(self, netlist):
        ...
        # Get the number of nodes for creating Yn and In
        n, content = self.get_number_of_nodes(content) 
        Yn = np.zeros((n, n), dtype = 'complex_')
        In = np.zeros((n, 1), dtype = 'complex_')

        # Fills Yn and In accordingly to the netlist 
        Yn, In = self.fill_matrixs(Yn, In, content)
